...
# 创建权重参数列表
WEIGHTS=("$model_name")  # 将起始权重参数添加到列表中
...
# 创建保存测试精度信息的文件
result_file="model_weights/$dataset_name/$model_name/${file_prefix}.txt"
> $result_file
...
do
    ...
    # 更新权重参数
    sed -i "s/test_weights_name = \".*\"/test_weights_name = \"$weight\"/g" ./config/$dataset_name/$model_name.py

    # 执行测试命令，并将精度信息追加写入文件
    echo "执行测试，权重参数为: $weight.ckpt"
    echo "------------------------------------------------------"
    result=$(python ./building_seg_test.py -c ./config/$dataset_name/$model_name.py -o fig_results/$dataset_name/$model_name --rgb -t 'lr')
